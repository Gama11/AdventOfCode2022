...
class Day13 {
	...
	public static function sumCorrectlyOrderedIndices(input:String):Int {
		function compare(p1:Packet, p2:Packet):Decision {
			return switch [p1, p2] {
		...
		return parsePacketPairs(input).mapi((index, pair) -> {index: index + 1, pair: pair})
			.filter(it -> switch compare(it.pair[0], it.pair[1]) {
				case Correct: true;
				case Incorrect: false;
				case Continue: throw "missing decision";
			})
			.map(it -> it.index)
			.sum();
	}
